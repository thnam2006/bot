    ...
    try {
        ...
        switch (logMessageType) {
            ...
            case "log:thread-name": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`[ THÔNG BÁO ] | Nhóm cute này đã được cập nhật tên thành ${dataThread.threadName}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 2000));
                        return api.unsendMessage(info.messageID);
                    } else return;
                });
                break;
